...
app.use((err, req, res, next) => {
    ...
    if (err.name === 'ValidationError') {
        ...
        return res.status(400).json({ error: messages.join(', '), message: messages.join(', ') });
    }

    if (err.code === 11000) {
        return res.status(400).json({ error: 'This email is already registered.', message: 'This email is already registered.' });
    }
